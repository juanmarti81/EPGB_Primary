...
df_datos = pd.DataFrame(
    {
        'ticker': tickers['ticker'].to_list(),
        'symbol': tickers['symbol'].to_list()
    },
    columns=[
        'ticker', 'symbol', 'bidsize', 'bid', 'ask', 'asksize', 'last',
        'close', 'open', 'high', 'low', 'volume', 'lastupdate'
    ])
df_datos = df_datos.set_index('ticker')

thisData = pd.DataFrame(columns=[
    'ticker', 'symbol', 'bidsize', 'bid', 'ask', 'asksize', 'last', 'close',
...
def calcular_opciones():
    ...
    calls['ratio_1'] = np.where(
        calls['ask'] == 0, 0,
        np.where(calls['bid_1'] == 0, 0,
                 ((calls['ask'] - calls['bid_1']) /
                  (calls['strike_1'] - calls['strike']))))
    calls['strike_dif'] = calls['strike_1'] - calls['strike']
    calls['dif_1'] = np.where(
        calls['ask'] == 0, 0,
        np.where(calls['bid_1'] == 0, 0, calls['ask'] - calls['bid_1']))
    calls['strike_2'] = calls['strike'].shift(-2)
    calls['bid_2'] = calls['bid'].shift(-2)
    calls['ratio_2'] = np.where(
        calls['ask'] == 0, 0,
        np.where(calls['bid_2'] == 0, 0,
                 ((calls['ask'] - calls['bid_2']) /
                  (calls['strike_2'] - calls['strike']))))
    calls['strike_dif_2'] = calls['strike_2'] - calls['strike']
    calls['dif_2'] = np.where(
        calls['ask'] == 0, 0,
        np.where(calls['bid_2'] == 0, 0, calls['ask'] - calls['bid_2']))
    calls.rename(columns={
        ...
        "dif_1": "Dif + 1",
        "dif_2": "Dif + 2",
        "strike_dif": "Dif Bases + 1",
        "strike_dif_2": "Dif Bases + 2"
    },
                 inplace=True)
    ...
    puts['ratio_1'] = np.where(
        puts['ask'] == 0, 0,
        np.where(puts['bid_1'] == 0, 0, ((puts['ask'] - puts['bid_1']) /
                                         (puts['strike'] - puts['strike_1']))))
    ...
    puts['ratio_2'] = np.where(
        puts['ask'] == 0, 0,
        np.where(puts['bid_2'] == 0, 0, ((puts['ask'] - puts['bid_2']) /
                                         (puts['strike'] - puts['strike_2']))))
...
# Add tick to datafrme. If it's an option go to calculate Bulls and bears
def addTick(symbol, bidSize, bid, ask, askSize, last, close, open, high, low,
            volume, lastUpdate):
    ...
    thisData = pd.DataFrame([{
        ...
        volume,
        'lastupdate':
        time.strftime('%m/%d/%Y %H:%M:%S', time.gmtime(lastUpdate / 1000.))
    }])
    thisData = thisData.set_index('ticker')
...
def market_data_handler(message):
    ...
    lastUpdate = None if not message['marketData']['LA'] else message[
        'marketData']['LA']['date']
    last = None if not message['marketData']['LA'] else message['marketData'][
        'LA']['price']
    bid = None if not message['marketData']['BI'] else message['marketData'][
        'BI'][0]['price']
    bidSize = None if not message['marketData']['BI'] else message[
        'marketData']['BI'][0]['size']
    ask = None if not message['marketData']['OF'] else message['marketData'][
        'OF'][0]['price']
    askSize = None if not message['marketData']['OF'] else message[
        'marketData']['OF'][0]['size']
    close = None if not message['marketData']['CL'] else message['marketData'][
        'CL']['price']
    open = None if not message['marketData']['OP'] else message['marketData'][
        'OP']
    high = None if not message['marketData']['HI'] else message['marketData'][
        'HI']
    low = None if not message['marketData']['LO'] else message['marketData'][
        'LO']
    volume = None if not message['marketData']['LO'] else message[
        'marketData']['EV']
    # print(symbol, bidSize, bid, ask, askSize, last, close, open, high, low, volume, lastUpdate)
    addTick(symbol, bidSize, bid, ask, askSize, last, close, open, high, low,
            volume, lastUpdate)
...
while True:
    try:
        if not df_datos.empty:
            shtData.range('A1').options(index=False,
                                        headers=True).value = df_datos
        if not calls.empty:
            shtOpciones.range('C3').options(index=False,
                                            headers=True).value = calls
        if not puts.empty:
            shtOpciones.range('V3').options(index=False,
                                            headers=True).value = puts
        time.sleep(1)
    except AssertionError as error:
        print(
            'Error de inserción. Algún dataframe esta vacío o generó un error ',
            error.args, error)
